...
def admin_ads(request):
    ...
    for dat in ads:
        ...
        for datas in detail:
            ...
            adatas['userid']=dat.id
            lists.append(adatas)


    return render(request,'admin_ads.html',{'li':lists})

def admin_approve(request,uid,pid):
    if request.session.is_empty():
        return redirect(admin_sign)
    a=request.session['local']

    data=db.collection('advertiser').document(uid).collection('advertises').document(pid).get().to_dict()
    persons= db.collection('advertiser').document(uid).collection('personal').get()
    names= db.collection('user').document(a).get().to_dict()
    print("dss")
    print(data)
    print(persons)
    ...
    return render(request,'admin_approve.html',{'k':data,"per":pdict})


def ad_approve(request,uid,pid):
    db.collection('advertiser').document(uid).collection('advertises').document(pid).update({'status':"Approved"})
    ...
    #print(list)
    #return render(request,'admin_approve.html',{'pro':list})
